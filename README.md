...
- 2026-02: Online hotfix: трасса в online-режиме теперь всегда рендерится по `trackId` (с fallback на `canyon_loop`), онлайн-игроки рисуются как змеи (голова+сегменты), а не как точки.
- 2026-02: Online net-sim hotfix: в `match-server` разрешён старт заезда даже при 1 участнике (ready=true на join); автогаз для человека убран, движение снова только от ввода игрока.
- 2026-02: Online steering visual fix: в рендере online-режима учтён поворот игрока (`turn`) через `heading` из snapshot — теперь змея смещается по ширине трассы и визуально рулится, а не едет по центрлайну как «на рельсах».
- 2026-02: Online control fix: змеи в online теперь рендерятся по реальным `x/y/heading` из серверного snapshot (без привязки к центрлайну), поэтому удержание `D`/`A` даёт непрерывный 360° поворот по/против часовой.
- 2026-02: Match-server spawn fix: стартовые позиции online-игроков привязаны к выбранной трассе (`canyon_loop`/`switchback_run`/`twin_fang`) с небольшим разносом по сетке, чтобы управление сразу происходило на видимой карте.
- 2026-02: Online steering tuning: усилена рулежка (выше базовая скорость поворота и быстрее реакция руля), при этом сохранено сглаживание, чтобы поворот оставался плавным без дискретной «лесенки».
- 2026-02: Online object generation fix: в online-рендере добавлена генерация объектов на трассе (пикапы `BOOST/SHIELD/OIL/BOMB` по `pickupFractions`, а также `APPLE/CACTUS` с разносом и проверками минимальных дистанций до чекпоинтов/пикапов/других объектов).
- 2026-02: Online room selection: на экране выбора трассы в online-режиме добавлен выбор комнаты (выпадающий список доступных roomId + ручной ввод roomId + кнопка обновления списка).
- 2026-02: Online input reliability fix: сервер `match-server` теперь принимает `input` не только в `running`, но и в `lobby/countdown` (кэш до старта), а фронтенд отправляет онлайн-ввод не только по keydown/keyup, но и периодическим input-pump, чтобы управление не «терялось» при старте.
- 2026-02: Offline control fix: anti-stall больше не перехватывает рулежку у игрока (только поддерживает минимальную скорость и grace-window), а автодоводка курса при глубоком `outside` оставлена только ботам.
- 2026-02: UX fix: режим оффлайн по умолчанию переключен на `CLASSIC` (1 игрок + 3 бота), чтобы старт всегда начинался с ручного управления игроком.
...
- 2026-02: Online gameplay sync fix: object interaction moved to server-authoritative simulation (`match-server/src/raceRoom.js`) with pickup/body-item collision, temporary effects, respawn timers, and snapshot object states; online renderer now consumes `snapshot.pickups/bodyItems` so collected objects visibly disappear and reappear on respawn (`src/game/scene.js`).
- 2026-02: Online track compliance fix: server simulation now applies road/offroad/outside surface penalties and outside recovery pull/steer using real track projection, so snakes no longer freely drive as if track boundaries did not exist (`match-server/src/raceRoom.js`).
- 2026-02: Online race freeze fix: disabled implicit auto-finish by lap count in `match-server` default flow (now `lapsToFinish=0` unless explicitly set), so players do not freeze after 2-3 laps; race ends by timeout/manual flow instead (`match-server/src/raceRoom.js`).
...
- 2026-02: Online finish flow fix: при `snapshot.phase=finished` клиент теперь сразу открывает экран результатов и формирует итоговую таблицу (финишировавшие — по времени, остальные — по прогрессу), без ручного перехода (`src/game/uiFlow.js`).
- 2026-02: Leaderboard MVP: подключен рабочий экран таблицы лидеров в меню и API `GET /match/leaderboard/:trackId` (через Nakama RPC `get_track_leaderboard`) с отображением топа по лучшему времени (`src/game/uiFlow.js`, `src/game/onlineRoomClient.js`, `match-server/src/index.js`, `match-server/src/nakamaClient.js`, `backend/nakama/modules/main.lua`).
- 2026-02: Anti-freeze online finish: если в online-гонке длительное время нет прироста прогресса, сервер завершает заезд по прогрессу (с расчетным `finishTimeMs` для итоговой сортировки), чтобы игроки не зависали бесконечно на трассе (`match-server/src/raceRoom.js`).
- 2026-02: Online results fallback: клиент в online теперь переводит в экран результатов не только по `phase=finished`, но и по устойчивому stall-fallback (долгое отсутствие движения), чтобы таблица всегда всплывала после фактического конца заезда (`src/game/uiFlow.js`).
- 2026-02: Match-server debug logging: добавлены подробные логи жизненного цикла комнаты (`room_created`, join/leave, phase transitions, `tick_summary`, причины `force_finish`) с управлением через env `RACE_DEBUG_LOGS` и `RACE_DEBUG_EVERY_TICKS` (`match-server/src/raceRoom.js`, `docker-compose.yml`).
- 2026-02: Перед заездом добавлена обязательная фаза `Правила` (2 секунды) с крупным оверлеем: цель гонки — пройти 3 круга на максимальной скорости; затем автоматически запускается отсчёт `3..2..1` и старт.
- 2026-02: В оффлайн и онлайн заездах финальная дистанция теперь считается как `3 круга` по фактической длине выбранной трассы (без искусственного завышения длины круга), а в snapshot онлайн-комнаты добавлены поля prestart/countdown таймингов.
- 2026-02: В главное меню добавлена кнопка `Правила` и модальное окно с краткой целью игры и карточками всех объектов на трассе (`BOOST`, `SHIELD`, `APPLE`, `OIL`, `BOMB`, `CACTUS`) с иконками из ассетов и краткими свойствами/длительностью эффектов.
- 2026-02: Окно `Правила` расширено пояснением по длине тела змеи: длинное тело помогает держать скорость, уменьшение тела усиливает замедление и повышает риск проигрыша по времени.
- 2026-02: Кнопка `Правила` перенесена на первое место в главном меню; блок меню центрирован и шрифт кнопок увеличен для лучшей читаемости.
- 2026-02: Исправлен стиль выпадающего списка трасс в экране лидерборда: тёмный фон и контрастный светлый текст для `select/option`.
- 2026-02: Мобильный UI: игровое поле фиксирует исходное соотношение сторон без растягивания трассы в landscape, HUD-блоки прижаты к краям экрана, добавлен экранный тач-джойстик (газ/тормоз/поворот) для управления змеёй.
- 2026-02: Последняя кнопка главного меню переименована в `Об игре`; добавлено отдельное модальное окно с историей проекта/авторами и иллюстрацией `assets/Xak/Xakathon.png` (`index.html`, `src/game/uiFlow.js`, `style.css`).
- 2026-02: Исправлен favicon: `favicon.ico` переконвертирован в настоящий ICO (мультиразмер 16/32/48/64), в `head` добавлены явные теги `rel="icon"`/`rel="shortcut icon"` с версионированным URL для обхода браузерного кэша, а в nginx добавлено отдельное правило `location = /favicon.ico` с `no-store` (`index.html`, `favicon.ico`, `infra/web/nginx.conf`).
