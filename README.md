...
- после `3` шагов голода подряд — `Сход (истощение)`;
...
- `BOMB`: радиусный дебаф замедления.
- `VENOM_SLOW` (яд): змеи могут плеваться ядом; у каждого типа своя дальность/кулдаун/сила замедления.
- `яблоко`: +1 сегмент тела, сброс шага голода.
- `кактус`: -1 сегмент тела.
- Истощение: каждые 12 секунд без яблока -1 сегмент; после 3 голодных тиков подряд — `Сход`.
